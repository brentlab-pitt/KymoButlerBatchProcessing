...
{dim=ImageDimensions@kym,coloredlines,overlay,overlaylabeled,labels,c,ptmp,sel,ovlpIDs,out,paths,pathstmp,inflp,t,seedbin,seeds,allyx,ptrk,trks,checkifAnyTrkisSubset,ovlpSegID},
	...
	(*Directions per track*)
	direct = Map[
	  If[
	    Length[#] >= 2 && AllTrue[#, ListQ],
	    Sign[#[[-1, 2]] - #[[1, 2]]], (* direction *)
	    Missing["NotApplicable"]  (* or some other default value indicating the structure is not as expected *)
	  ] &,
	  trks
	];
			...
			(*coloredlines = ImageRotate[
			  Image[
			    Show[
			      ...
			      Graphics@MapIndexed[
			        {
			          If[direct[[First@#2]] == 1, Pink, 
			            If[direct[[First@#2]] == -1, Green, Blue]], 
			          Style[Line@#1, Antialiasing -> False]
			        } &, 
			        Map[# - {1, 0} &, trks, {2}]
			      ]
			    ],
...
(*BiKymoButlerTrack[pred_, kym_, kympreproc_, bool_, binthresh_, vthr_, vismod_, minSz_, minFr_, colorChoice_, debug_: False] :=Module[
{dim=ImageDimensions@kym,coloredlines,overlay,overlaylabeled,labels,c,ptmp,sel,ovlpIDs,out,paths,pathstmp,inflp,t,seedbin,seeds,allyx,ptrk,trks,checkifAnyTrkisSubset,ovlpSegID},
	Sow[First@AbsoluteTiming[
	out=ImageResize[Binarize[pred,binthresh],dim];
	out=SmoothBin@SmoothBin@out;
	...
	Sow[First@AbsoluteTiming[
		...
		(* Colored lines and overlays *)
		coloredlines = 
		  If[colorChoice === "directional", (* Conditional for color choice *)
			  Print["Using directional colors"];
			  ImageRotate[
			    Image[
			      Show[
			        Image@Table[0, dim[[1]], dim[[2]]],
			        Graphics@MapIndexed[
			          {
			            If[First@#2 <= Length[directions],
			              If[directions[[First@#2]] == 1, Pink, Green],
			              RandomColor[]
			            ],
			            Style[Line@#1, Thickness[0.003], Antialiasing -> False] (* Thicker overlay *)
			          } &,
			          Map[# - {1, 0} &, trks, {2}]
			        ]
			      ]
			  ...
			  (* If colorChoice is not "directional", use random colors *)
			    ...
			    Image[
			      Show[
			        ...
			        Graphics@Map[
			          {
			            RandomColor[],
			            Style[Line@#, Thickness[0.003], Antialiasing -> False] (* Thicker overlay *)
			          } &,
			          Map[# - {1, 0} &, trks, {2}]
			        ]
			      ]
	...
	BiKymoButler[kym_, binthresh_, vthr_, tD_, cnet_, vismod_, minSz_, minFr_, colorChoice_, debug_: False] := Module[
	{rawkym,kympreproc,pred,bool,t,out},
	{t,{bool,rawkym,kympreproc,pred}}=AbsoluteTiming@BiKymoButlerSegment[kym,cnet,tD];
	If[debug,Sow[pred,"pmap"]];
	Sow[t,"seg"];
